...
export default function CommunityPage() {
  ...
  const fetchUserCommunities = useCallback(async () => {
    ...
    try {
      const fetchedCommunities = await getUserCommunities(session?.profile.id);
      setUserCommunities(fetchedCommunities || null);
    } catch (error) {
      console.error(error);
  ...
  const fetchCommunities = useCallback(async () => {
    if (userCommunities === undefined) return;
    try {
      const fetchedCommunities = await getCommunities();
      setCommunities(
        userCommunities
          ? fetchedCommunities?.filter(
              (community) =>
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    {userCommunities.map((community) => {
                      const memberProfile = community.members.find(
                        (member) => member.userId === session.profile.id
                      );
                      return (
                        <CommunityCard
                          key={community.id}
                ...
                {communities && communities.length > 0 ? (
                  ...
                  <Card className="border-dashed border-2 border-celestial-blue-50">
                    <CardContent className="flex flex-col items-center justify-center py-12 sm:py-16 text-center px-4">
                      ...
                      <p className="text-sm sm:text-base text-muted-foreground mb-6 max-w-md">
                        Create a community to connect with people who share your
                        interests
                      </p>
                      <div className="flex flex-col sm:flex-row gap-3">
                        <Link href="/community/create">
